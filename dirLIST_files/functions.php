...
function array_sum_($array) {
    return is_countable($array) ? array_sum($array) : 0;
}
    ...
    if ($listing_mode == 0) {
        ...
        foreach ($content as $key => $val) {
            if (!in_array($val, $exclude)) {
                ...
                if (is_dir($item_path)) {
                    $folders['name'][] = $val;
                    $folders['date'][] = date("d F Y", filemtime($item_path));
                    ...
                    if (!($val == $this_file_name && $this_file_size == $file_size)) //Exclude the main index file specifically
                        ...
                        if (!in_array($file_ext, $exclude_ext)) {
                            ...
                            $files['date'][] = date("d F Y", filemtime($item_path));
                            if ($images_detected == '')
                                $images_detected = (in_array(strtolower($file_ext), array(
                                    '.jpeg',
                                    '.jpg',
                                    '.png',
                                    '.gif'
        ...
        if (!in_array(substr($content[0], 0, 1), array(
            'd',
            '-'
...
{
    $file_size = max($byte_size / 1024, 0);
...
function icons($files, $view_mode)
{
    ...
    foreach ($files as $key => $val) {
        ...
        elseif (in_array($file_ext, $nyt))
            $files_icons[$key] = 'url-nyt' . $icon_ext;
...
function set_local_text($language_id)
{
    ...
    */
    ...
    $text = array(
        ...
        'switch_to_list' => array(
            'Switch to list view',
            'Passez à vue de liste',
            'Umschalten auf Listenansicht',
            'Cambiar a vista de lista'
        ),
        
        'switch_to_thumbnail' => array(
            'Switch to thumbnail view',
            'Switch to vignette',
            'Umschalten auf Symboldarstellung ',
            'Cambiar a vista de miniaturas'
        ),
        
        'launch_gallery' => array(
            'Launch Gallery',
            'Lancer la galerie',
            'Start Galerie',
            'Lanzamiento de la galería'
        ),
        
        'launch_media_player' => array(
            'Launch Media Player',
            'Lancement media player',
            'Starten Sie Media Player ',
            'Lanzamiento Media Player'
        ),
        
        'show_hide_stats' => array(
            'Show/hide statistics',
            'Afficher/cacher les statistiques',
            'Zeigen/verstecken Statistiken',
            'Mostrar/ocultar las estadísticas'
        ),
        ...
        'this_page_loaded_in' => array(
            'This page loaded in',
            'Cette page chargée dans',
            'Diese Seite wurde geladen in',
        ...
        'no_go_back' => array(
            ...
            'Diese Aktion ist nicht mehr rückgängig zu',
        ...
        'check_for_update' => array(
            ...
            'Siehe, wenn Update verfügbar ist',
            '	
Comprobar actualización '
        ...
        'no_update_found' => array(
            'No Update Found',
            'Pas de mise à jour introuvable',
